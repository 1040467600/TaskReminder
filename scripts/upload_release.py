...
RELEASE_NOTES = """## v2.0.7 日期控件体验优化

- **日期框等宽**：搜索栏创建/截止时间、提醒历史的开始/结束日期框统一固定宽度 130px，不再一长一短
- **翻年按钮**：每个日期框两侧新增 « » 双箭头按钮（上一年/下一年），与日历弹出框内的 ◀ ▶ 单箭头（翻月）明确区分；hover 高亮，带 tooltip
- **提醒历史默认当天**：快捷范围选"全部"时，起始日期不再重置为 2000 年，而是保持当前显示值（查询时不限制起始），日历弹出始终从当前月份开始
- **日期框美化**：QDateEdit 固定宽度、圆角、内边距、下拉按钮（日历图标）左右分隔线，浅色/深色主题适配
- 全量 250 项测试通过，E2E 走查 66 项通过
